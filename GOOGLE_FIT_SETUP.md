...
1. **Включите Google Calendar API** в [Google Cloud Console](https://console.cloud.google.com/) → APIs & Services → Library → **Google Calendar API** → Enable.
2. В **Настройках** приложения откройте блок **«Синхронизация с Google Календарь»**.
3. Включите переключатели: **Подцели**, **Привычки**, **Цели** — что выгружать.
4. Нажмите **«Выгрузить в календарь»**. События появятся в основном календаре Google.
